...
Deconvolution <- function(T, C, method, phenoDataC, P = NULL, elem = NULL, STRING = NULL, marker_distrib, refProfiles.var){ 
    ...
    if(method %in% bulk_methods){
        ...
        require(xbioc)
        C.eset <- Biobase::ExpressionSet(assayData = as.matrix(C),phenoData = Biobase::AnnotatedDataFrame(phenoDataC[colnames(C),]))
    ...
    if(method=="CIBERSORT"){ #without QN. By default, CIBERSORT performed QN (only) on the mixture.
        ...
        require(dtangle)
        mixture_samples = t(log(T+1))
        reference_samples = t(log(C+1))
        reference_samples = reference_samples[row.names(reference_samples) %in% marker_distrib$CT,]
