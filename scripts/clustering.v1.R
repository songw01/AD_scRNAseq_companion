...
pobj = ggplot() + geom_point(data = as.data.frame(gene.var[,1:6]),aes(x = tech,y = bio,colour = FDR < 0.05),alpha = 0.2) + 
  ...
  geom_text_repel(data = subset(data.frame(gene.var[1:20,1:6],gene = rownames(gene.var)[1:20]),FDR < 0.05),aes(x = tech,y = bio,label = gene),size = 5) + 
...
png(file = "variance.png",res = 300,width = 1800,height = 2100)
print(pobj)
dev.off()

############### PCA data
pcs <- getDenoisedPCs(t(Seurat::GetAssayData(seu, assay = "SCT", slot = "scale.data")), technical=gene.var)

# Get the total variance:
total_variance <- sum(matrixStats::rowVars(Seurat::GetAssayData(seu, assay = "SCT", slot = "scale.data")))

pca.pdata = data.frame(PC = 1:length(pca@stdev),
...
pobj = ggplot(data = pca.pdata,aes(x = PC,y = varExplained)) + geom_point() + geom_line() + 
  geom_vline(xintercept = c(ncol(pcs$components),7,20),colour = c("red","blue","green")) + 
  theme_bw() + 
  labs(x = "PCs",y = "%. Variance Explained") + 
  theme(axis.title = element_text(size = 20),axis.text = element_text(size = 17))

png(file = "variance_explained.png",res = 300,width = 2400,height = 2100)
print(pobj)
dev.off()
...
png(file = "umap.png",res = 300,width = 3500,height = 2800)
print(pobj)
dev.off()
...
png(file = "Celltype_Markers.Dotplot.png",res = 300,width = 1700,height = 2700)
print(dobj)
dev.off()
